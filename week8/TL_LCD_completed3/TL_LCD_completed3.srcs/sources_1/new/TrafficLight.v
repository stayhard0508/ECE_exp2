...
always @(posedge clk or negedge rst) begin
    if (!rst) begin
        ...
        if (btn_stop_trig == 1) begin //if press button stop 
            ...
            down_count_reg <= down_count;
            state <= state_reg; // 저장된 상태로 이동
        end  
        // 주간/야간 상태 전이
        else if (count == 10) begin // 5초 50000000            
            if (hours >= 8 && hours < 23) // 주간
                case (state)
                    A: if(down_count == 0) //주간의 수동조작state가 5초가 되는 것을 방지 
                    state <= D;
                    D: state <= F;
                    F: 
                        begin
...
begin
    if(!rst) begin count <= 4'b0000;
        ...
        case (state)
            A: 
                begin
                led_red <= 4'b0011; //0011
                led_green <= 4'b1100; //1100
                led_left <= 4'b0000; // 0000
                if (hours < 8 || hours >= 23) begin// 야간
                    
                    if (A_count == 0) begin // 야간 황색 신호 처리1
                        if (count >= 6 && count <= 9) // 5S
                            led_yellow <= 4'b1000; //1000
                        else if (count > 9) // 10S
                            led_yellow <= 4'b0000;
                    end
                    else begin // 야간 황색 신호 처리2
                        if (count >= 6 && count <= 9) // 5S
                            led_yellow <= 4'b1100; //1100
                        else if (count > 9) // 10S
                            led_yellow <= 4'b0000;
                    end
                end   
                ...
                // 보행자 신호 처리
                if (count >= 5 && seconds > seconds_reg) begin // 5S 이상 이면 점멸신호
            ...
            F: 
                ...
                // 보행자 신호 처리
                if (count >= 5 && seconds > seconds_reg) begin // 5S
            ...
            G: 
                ...
                // 보행자 신호 처리
                if (count >= 5 && seconds > seconds_reg) begin // 5초
                    led_walk_green[2] <= ~led_walk_green[2];
                    end
                //else if (pedestrianCount >= 5) 
                    //pedestrianCount <= 0;                    
