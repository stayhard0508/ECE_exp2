...
always @(posedge clk or posedge rst) begin
    if (rst) begin
        ...
        // 주간/야간 상태 전이
        if (count == 5) begin // 5초 50000000
            if (current_time >= 80000 && current_time < 230000) // 주간
                case (state)
                    A: state <= D;
                    D: state <= F;
                    F: 
        ...
        case (state)
            A: 
                ...
                led_green <= 4'b1000;
                led_left <= 4'b0000;
                if (current_time < 80000 || current_time >= 230000) begin// 야간
                    
                    if (A_count == 0) begin // 야간 황색 신호 처리1
                        if (count == 4) // 4.9초 49000000
                            led_yellow <= 4'b1000;
                        else if (count == 5) // 5초
                            led_yellow <= 4'b0000;
                    end
                ...
                // 보행자 신호 처리
                if (count >= 3) begin // 2.5초 25000000
